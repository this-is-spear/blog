...
비동기 요청은 스프링에서 지원하는 @Async 애너테이션을 이용해 쉽게 달성할 수 있다.

```kotlin
@Async("asyncSendExecutor")
fun sendNotification(pendingNotification: PendingNotification) {
    // 요청 진행
}
```
...
class AsyncConfiguration {
    @Bean
    fun asyncSendExecutor(chunkSizeProperties: ChunkSizeProperties): Executor {
        return ThreadPoolTaskExecutor().apply {
          corePoolSize = chunkSizeProperties.asyncCorePoolSize
          maxPoolSize = chunkSizeProperties.maxPoolSize
          queueCapacity = chunkSizeProperties.asyncQueueCapacity
          // 처리량이 많아 실패하는 경우 대응책을 정한다. CallerRunsPolicy, AbortPolicy, DiscardPolicy 가 존재한다.
          setRejectedExecutionHandler(ThreadPoolExecutor.CallerRunsPolicy())
          // 작업이 종료될 때 까지 대기한다.
          setWaitForTasksToCompleteOnShutdown(true)
          // 진행 중인 작업이 완료되는 동안 Spring의 컨테이너 종료가 계속되는데 작업이 완료될 때까지 컨테이너 종료를 대기하도록 설정한다.
...
- corePoolSize : 작업을 처리하는 작업자들 크기
- maxPoolSize : 작업을 처리하는 작업자들의 한계치
- queueCapacity : 처리할 작업이 대기 가능한 허용량

어렵게 생각들 수 있는데 작업이 들어왔을 때 다음처럼 행동한다.
...
거절한다면 다음과 같은 방식으로 작업한다.

- CallerRunsPolicy : 호출자가 직접 실행
- AbortPolicy : TaskRejectedException 예외 발생! (기본값)
- DiscardPolicy : 무시해버림

종료 시키지 않으려면 다음 옵션을 설정해야 한다.

- waitForTasksToCompleteOnShutdown : 작업이 전부 실행될 때까지 종료하지 않는다.
- awaitTerminationSeconds : 작업이 전부 실행됨을 확인하고 대기하는 시간
...
- 요청을 보낼따마다 스레드를 생성한다면 리소스 낭비다.
- 읽는 속도가 빠르고 읽는 양도 많아졌지만 처리율은 그대로다.

두 가지에 대해서 어떻게 해결할지 설명해보겠다.
