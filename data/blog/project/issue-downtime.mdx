...
유레카 서버 로그에서는 인스턴스가 새로 생성되고 종료됨이 반복하고 있었고 그로 인해 다운타임 영향도와 간격이 길어졌다. 생성 종료가 반복되는 현상을 인프라팀과 이야기를 나눴고, ecs에서 일정 시간동안 health check가 되지 않으면 재시작함을 알게 됐다.

즉, 컨텍스트가 뜨기 전에 `health check` 날리고 있는게 원인이고 기능 추가가 늘어나면서 애플리케이션 컨텍스트가 올라가는 시간 증가가 근본적인 문제였다.
...
사용자 요청에 대한 connection timeout 이 아니라 internal service 통신에 설정할

추가 기능으로 배포 과정을 개선해야 했다.

- 해결방안 1 → 코드 수정 영향도를 줄인다. → 게이트웨이 수정 필요
  - 캐싱 시간을 줄인다. +
  - connection timeout을 줄인다.
  - retry를 수행한다.
- 해결방안 2 → 배포 프로세스를 개선한다. → actuator 활용
  - actuator를 이용해 서비스 상태를 변경할 수 있도록 수정한다.
  - 배포 과정에서 api를 호출해 서비스를 제어한다.

![그림 6](/static/images/project/issue-downtime/6.png)

서비스 레지스트리에서 직접 관리하는 방법 → 레지스트리 즉각 반영됨. 서비스 상태와 sync 안맞음

- 문제에 대한 접근
  - 실제 서비스 상태가 다르다. → 레지스트리에 등록된 서비스 상태가 먼저 변경돼면 문제가 없다.
...
해당 방식은 유레카 서버에게 즉각적인 상태 반영은 가능하지만 서비스 상태와 달라 sync 맞추기가 어렵다. 만약 `spring actuator`가 제공하는 `health check`와 유레카 서버 상태 간 sync를 맞추려면 다음 방법이 좋다.

- 해결방안 3 → 배포 프로세스를 개선한다. → health check 와 eureka staus 간 sync 맞추기
...
`spring actuator`를 사용하는 경우 `/actuator/pause`, `/actuator/resume` 메서드를 이용할 수 있는데, PauseHandler 를 구현해 내부 기능을 채운다면 쉽게 제어가능하다.
