...
sequenceDiagram
    ...
    rect rgb(200, 130, 155)
    Note over Thread1,PubSub: Phase 1: 첫 번째 스레드 락 획득 시도 및 획득
    Thread1->>RLock: tryLock()
    RLock->>RedissonLock: lockAsync()
    RedissonLock ->> Redis : tryAcquire(EVAL)
    Redis -->> RedissonLock : success
    RedissonLock-->>RLock:
    RLock-->>Thread1: acquire lock
    end

    rect rgb(255, 130, 100)
    Note over Thread1,PubSub: Phase 2: 두 번째 스레드 락 획득 시도 및 대기
    Thread2->>RLock: tryLock()
    RLock->>RedissonLock: lockAsync()
    RedissonLock ->> Redis : tryAcquire(EVAL)
    Redis -->> RedissonLock : fail
    RedissonLock->>+PubSub: subscribe with timeout
    end
    Note over PubSub: 타임 아웃 만큼 대기

    rect rgb(230, 155, 100)
    Note over Thread1,PubSub: Phase 3: 첫 번째 스레드 락 해제 및 해제 신호 전파
    Thread1->>RLock: unlock()
    RLock->>RedissonLock: unlockAsync()
    RedissonLock ->> Redis : unlockInnerAsync(EVAL)
    Redis -->> RedissonLock : success
    RedissonLock->>PubSub: publish(unlock)
    PubSub -->> RedissonLock:
    RedissonLock-->>RLock:
    RLock-->>Thread1: release lock
    end

    rect rgb(255, 100, 130)
    Note over Thread1,PubSub: Phase 4: 두 번째 스레드 락 해제 신호 전달 및 락 획득 시도
    PubSub-->>-RedissonLock: notify
    RedissonLock ->> Redis : tryAcquire(EVAL)
    Redis -->> RedissonLock : success
    RedissonLock-->>RLock:
    RLock-->>Thread2: acquire lock
    end
```
...
controller ->> request-service: start transaction
activate request-service
request-service ->> subscriber: await
subscriber ->> redis: subscribe
request-service ->> producer: send message
producer ->> redis: produce
redis -->> producer:
producer -->> request-service:

response-service ->> consumer: start consume
consumer ->> redis: get transaction
redis -->> consumer:
consumer -->> response-service :
response-service  ->> repository: save transaction
repository -->> response-service:
response-service ->> consumer: end consume
consumer ->> redis: ack
redis -->> consumer:
consumer -->> response-service:
response-service ->> publisher: singal
publisher ->> redis: publish
redis -->> publisher:
publisher -->> response-service:

redis -->> subscriber: receive response
subscriber -->> request-service: signal
request-service -->> controller: end transaction
deactivate request-service
controller -->> user: 200OK
```
...
box rgb(100,134,134) producer
participant controller
participant send-reply-service
end

participant rabbitmq

box rgb(100,134,134) consumer
participant receive-request-service
participant repository
end

user ->> controller: request
controller ->> send-reply-service: start transaction
send-reply-service ->> rabbitmq: send
rabbitmq -->> receive-request-service:
receive-request-service ->> repository: save transaction
repository ->> receive-request-service:
receive-request-service ->> rabbitmq: receive
rabbitmq -->> send-reply-service:

send-reply-service ->> controller: end transaction
controller -->> user: 200OK
```
