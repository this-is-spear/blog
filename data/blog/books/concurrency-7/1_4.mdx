...
레디스 관련 정보 글은 아니니 추가 설명은 진행하지 않겠다.
...
1. XREADGROUP 가장 많이 호출됐으며 총 실행 시간은 5.2초다.
...
분산락의 한계를 보완하기 위해 생산자 소비자 패턴 적용을 고려하게 됐는데 인프라까지 손대며 복잡도를 높이고 싶지 않았다.
그래서 `Redis streams`를 고려했지만 단일 작업자로 동작하기 때문에 소비 작업이 생산 작업에 영향이 가며 생산자 소비자 패턴에 적용하기에는 부적하다는 것을 알게 됐다.
