...
아직 게이트웨이에는 `if-none-match`와 `if-modified-science`를 식별 할 수 있는 기능을 제공하고 있지 않는 듯 해.
음... 의도가 있었을 듯 한데 아직까지는 파악하지 못했어. 관련 문서를 조금 더 찾아보고 키 값에 식별 할 수 있도록 수정 할 수 있어보여.
